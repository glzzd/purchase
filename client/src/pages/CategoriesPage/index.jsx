...
const Categories = () => {
  ...
  // Modal state'leri
  const [isMainCategoryModalVisible, setIsMainCategoryModalVisible] =
    useState(false);
  const [isSubCategoryModalVisible, setIsSubCategoryModalVisible] =
    useState(false);
  const [isProductModalVisible, setIsProductModalVisible] = useState(false);
  const [isSpecificationModalVisible, setIsSpecificationModalVisible] =
    useState(false);
  ...
  const fetchAllCategories = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5001/api/category/all-categories",
        {
          method: "GET",
          credentials: "include",
        }
      ...
      if (response.ok) {
        ...
        console.error(
          "Kateqoriyalar yüklənərkən xəta: Server yanıtı başarısız."
        );
      }
    } catch (error) {
      ...
      cellRenderer: (params) => {
        return (
          <Button
            ...
            onClick={() => openMoveModal(params.data)}
            type="text"
          >
            Yerdəyişmə
          </Button>
  ...
  const handleProductChange = async () => {
    try {
      const response = await fetch(
        "http://localhost:5001/api/category/update-product",
        {
          method: "PUT",
  ...
  useEffect(() => {
    if (newProduct.parentCategory) {
      const selectedCategory = categories.find(
        (cat) => cat._id === newProduct.parentCategory
      );
  ...
  useEffect(() => {
    if (newProduct.parentSubCategory && newProduct.parentCategory) {
      ...
      const selectedSubCategory = selectedCategory?.subCategories.find(
        (sub) => sub._id === newProduct.parentSubCategory
      );
      setFilteredProducts(
  ...
  useEffect(() => {
    if (newSpecification.parentCategory) {
      const selectedCategory = categories.find(
        (cat) => cat._id === newSpecification.parentCategory
      );
      setFilteredSubCategories(
        selectedCategory ? selectedCategory.subCategories : []
      );
  ...
  useEffect(() => {
    if (newSpecification.parentSubCategory && newSpecification.parentCategory) {
      ...
      const selectedSubCategory = selectedCategory?.subCategories.find(
        (sub) => sub._id === newSpecification.parentSubCategory
      );
      setFilteredProducts(
        selectedSubCategory ? selectedSubCategory.products : []
      );
  ...
  const removeSpecification = (index) => {
    const updatedSpecifications = newSpecification.specifications.filter(
      (_, idx) => idx !== index
    );
    setNewSpecification({
      ...newSpecification,
  ...
  const addMainCategory = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5001/api/category/add-main-category",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const addSubCategory = async () => {
    if (!newSubCategory.name.trim() || !newSubCategory.parent) {
      console.warning(
        "Zəhmət olmasa, ana kateqoriya və alt kateqoriya adını daxil edin!"
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        "http://localhost:5001/api/category/add-sub-category",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            name: newSubCategory.name,
            mainCategoryId: newSubCategory.parent,
          }),
        }
      );
  ...
  const addProduct = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5001/api/category/add-product",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...
            mainCategoryId: !newProduct.parentSubCategory
              ? newProduct.parentCategory
              : null, // Alt kategori yoksa ana kategoriye ekle
          }),
        }
      );
      ...
      if (response.ok) {
        ...
        message.error(
          `Məhsul əlavə edilərkən xəta baş verdi: ${
            errorData.message || "Naməlum xəta"
          }`
        );
      }
    } catch (error) {
  ...
  const addSpecification = async () => {
    if (
      !newSpecification.parentProduct ||
      newSpecification.specifications.length === 0
    ) {
    ...
    try {
      const response = await fetch(
        "http://localhost:5001/api/category/add-specification",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  return (
    <div className="bg-white rounded-md p-4 flex flex-col">
      ...
      <Modal
        title="Yeni Ana Kateqoriya"
        open={isMainCategoryModalVisible}
        onOk={addMainCategory}
        onCancel={() => setIsMainCategoryModalVisible(false)}
      >
        <Input
          placeholder="Ana kateqoriya adı"
      ...
      >
        <div className="flex flex-col gap-2">
          <Select
            ...
            style={{ width: "100%" }}
            onChange={(value) =>
              setNewSubCategory({ ...newSubCategory, parent: value })
            }
          >
            {categories.map((cat) => (
      ...
      >
        <div className="flex flex-col gap-2">
          ...
          <Select
            ...
            onChange={(value) =>
              setNewProduct({ ...newProduct, parentSubCategory: value })
            }
          >
      ...
      >
        <Select
          ...
          onChange={(value) =>
            setNewSpecification({ ...newSpecification, parentCategory: value })
          }
        ...
        <Select
          ...
          onChange={(value) =>
            setNewSpecification({ ...newSpecification, parentProduct: value })
          }
